...
const FlightSearchPage: React.FC = () => {
  ...
  const { getDirectionalClasses, isRTL } = useRtlHelpers();
          ...
          <div className="space-y-4">
            {flights.map(flight => (
              <Card key={flight.id} className="hover:shadow-md transition-shadow">
                <CardContent className="p-6">
                  <div className={getDirectionalClasses(
                    "flex flex-col lg:flex-row justify-between gap-6",
                    "lg:flex-row-reverse"
                  )}>
                    {/* Airline Info */}
                    <div className={getDirectionalClasses(
                      "flex gap-4 items-center min-w-0 lg:w-64",
                      "flex-row-reverse"
                    )}>
                      <div className="w-16 h-16 bg-muted rounded flex items-center justify-center flex-shrink-0">
                        <Plane className="h-8 w-8 text-muted-foreground" />
                      </div>
                      <div className={getDirectionalClasses("min-w-0", "text-right")}>
                        ...
                        <div className="flex flex-col items-center justify-center space-y-2 w-full">
                          ...
                          <div className="w-full flex items-center justify-center">
                            ...
                            <div className="mx-2 p-1 bg-saudi-green/10 rounded-full">
                              <Plane className={getDirectionalClasses(
                                "h-4 w-4 text-saudi-green",
                                "h-4 w-4 text-saudi-green rotate-180"
                              )} />
                            </div>
                            <div className="h-[2px] bg-border flex-1"></div>
                          </div>
                    ...
                    {/* Price and Book */}
                    <div className={getDirectionalClasses(
                      "flex flex-col items-end justify-center space-y-4 lg:w-48",
                      "flex flex-col items-start justify-center space-y-4 lg:w-48"
                    )}>
                      <div className={getDirectionalClasses("text-right", "text-left")}>
                  ...
                  <div className="mt-6 pt-4 border-t">
                    <div className={getDirectionalClasses(
                      "flex flex-wrap gap-4 text-sm text-muted-foreground items-center",
                      "flex flex-wrap gap-4 text-sm text-muted-foreground items-center flex-row-reverse"
                    )}>
                      <div className="flex items-center">
                        <Badge variant="outline" className={getDirectionalClasses("mr-2", "ml-2")}>
